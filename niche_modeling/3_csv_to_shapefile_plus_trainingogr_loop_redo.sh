# Create vrt files
for i in ./powo_cleaned_redos/*.csv; do 
f=`echo $i | sed 's/\.csv//g' | sed 's/.*\///g'`
echo $f
cp aaa_coord.vrt ./powo_cleaned_redos/${f}.vrt
sed -i "s/output/${f}/g" ./powo_cleaned_redos/${f}.vrt
sed -i "s/coord/${f}/g" ./powo_cleaned_redos/${f}.vrt
sed -i "s/test\.csv/${f}.csv/g" ./powo_cleaned_redos/${f}.vrt
done
